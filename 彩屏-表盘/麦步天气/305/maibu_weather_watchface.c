...
//手表id
static char watch_id_buffer[14] = {0};
...
static uint32_t get_weather_icon(char * str,int32_t day_or_night)
{
	...
	else if(strstr(str,WEATHER_SUNNY) != NULL )
	{
	...
	else if(strstr(str,WEATHER_SNOW) != NULL)
	{
...
static void request_weather_info(char * city_name)
{
	...
	memset(watch_id_buffer,0,sizeof(watch_id_buffer));
	os_get_tag_serial_no(watch_id_buffer,sizeof(watch_id_buffer));
	...
	if(city_name != NULL)
	{
		...
		//	sprintf(url,"%s&did=%s",url,watch_id_buffer);	
	}
...
static int32_t set_temp_layerText(LayerText* temp_LayerText,char * str,GRect rect,enum GAlign alignment, uint8_t font_type,P_Window p_window,P_Layer p_layer,enum GColor color,enum GColor fcolor)
{
...
static P_Window init_window(void)
{
	...
	temp_frame.origin.x = 47;
	temp_frame.origin.y = 36;
	temp_frame.size.w = 87;
	temp_frame.size.h = 65;
	...
	/*添加小时分钟图层*/
	temp_frame.origin.x = 8;
	temp_frame.origin.y = 4;
	temp_frame.size.w = 55;
	temp_frame.size.h = 16;
	
	
	sprintf(buffer, "%02d:%02d", t.hour, t.min);
	set_temp_layerText(&temp_LayerText,buffer, temp_frame, GAlignLeft, U_ASCII_ARIAL_16,p_window,temp_P_Layer,GColorBlack,GColorWhite);


	/*添加星期月图层*/
	temp_frame.origin.x = 85;
	temp_frame.origin.y = 4;
	temp_frame.size.w = 83;
	temp_frame.size.h = 16;
	
	sprintf(buffer, "%02d-%02d %s", t.mon, t.mday, wday[t.wday]);
	set_temp_layerText(&temp_LayerText, buffer,temp_frame, GAlignRight,U_ASCII_ARIAL_16,p_window,temp_P_Layer,GColorBlack,GColorWhite);
	...
	if(g_weather_icon_key != 0)
	{
		...
		temp_frame.origin.x = 8;
		temp_frame.origin.y = 115;
		temp_frame.size.w = 24;
		temp_frame.size.h = 28;

		int8_t temp_temperature = g_temperature;
		if(g_temperature < 0)
		{
			temp_frame.size.w = 15;
			display_target_layer(p_window,&temp_frame,get_BIG_icon_key[10]);
			temp_frame.origin.x = temp_frame.origin.x + 15;
			temp_frame.size.w = 24;
		...
		temp_frame.origin.x = 58;
		temp_frame.origin.y = 154;
		temp_frame.size.w = 110;
		temp_frame.size.h = 16;

		sprintf(buffer,"%s",level_aqi_str[g_level]);
		set_temp_layerText(&temp_LayerText,buffer,temp_frame,GAlignRight,U_ASCII_ARIAL_16,p_window,temp_P_Layer,GColorWhite,GColorBlack);
		...
		set_temp_layerText(&temp_LayerText,buffer,temp_frame,GAlignRight,U_ASCII_ARIAL_20,p_window,temp_P_Layer,GColorWhite,GColorBlack);
	}
	else if(g_weather_icon_key == 0)
	{
		...
		temp_frame.origin.y = 36;
		temp_frame.size.w = 87;
		temp_frame.size.h = 80;
		display_target_layer(p_window,&temp_frame,RES_BITMAP_WATCHFACE_MAIBU_WEATHER_UPDATING);

		/*添加等待更新提示图层*/
		temp_frame.origin.x = 8;
		temp_frame.origin.y = 133;
		temp_frame.size.w = 137;
		temp_frame.size.h = 21;
