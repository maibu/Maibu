...
#define WEATHER_URL		"http://op.juhe.cn/onebox/weather/query?key=REPLACE_WITH_YOUR_KEY_HERE&cityname="
...
void request_weather_info(char * city_name)
{
	...
	maibu_comm_request_web(url,WEATHER_KEY,5*60*get_front_or_back_flag());
...
{
	maibu_comm_request_phone(ERequestPhoneSelfDefine,(void *)&get_city_context,34);
}
...
P_Window init_window(void)
{
	...
	int32_t bitmap_array[3] = {RES_BITMAP_WATCHFACE_WEATHER_BG,0,RES_BITMAP_WATCHFACE_WEATHER_UPDATING};
	...
	display_target_layer(p_window,&temp_frame,GAlignLeft,GColorWhite,bitmap_array,0);
   
	//添加天气图片
	//temp_frame = {{0, 28}, {60, 80}};
	*temp_frame_p = 0x503c1c00;
	g_weather_icon_key = get_weather_icon(weather_info,get_date_flag());
	bitmap_array[1] = g_weather_icon_key;
	display_target_layer(p_window,&temp_frame,GAlignLeft,GColorWhite,bitmap_array,1);
	...
	*temp_frame_p = 0x80641c00;
	g_layer_id_waiting = display_target_layer(p_window,&temp_frame,GAlignLeft,GColorWhite,bitmap_array,2);
...
void weather_comm_result_callback(enum ECommResult result, uint32_t comm_id, void *context)
{
	...
	if(maibu_get_ble_status() == BLEStatusConnected)
	{
		...
		if ((result == ECommResultFail) && (comm_id == g_comm_id_web_weather)&&(g_web_comfail_count < 5))
		{
			request_weather_info(g_city);
			g_web_comfail_count++;
		}
...
void watchapp_comm_callback(enum ESyncWatchApp type, uint8_t *buf, uint16_t len)
{
	...
	if(type == ESyncWatchAppUpdateParam)
    {
		...
		{
			bool rett;
			rett = app_persist_delete_data(CITY_PRESIST_DATA_KEY);
