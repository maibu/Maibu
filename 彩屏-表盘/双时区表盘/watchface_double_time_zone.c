...
#define TIMEZONE_BG_SIZE_H			176
#define TIMEZONE_BG_SIZE_W			176


/*地域1图层*/
#define TIMEZONE_ADDR1_ORIGIN_X			6
#define TIMEZONE_ADDR1_ORIGIN_Y			11
#define TIMEZONE_ADDR1_SIZE_H			16
#define TIMEZONE_ADDR1_SIZE_W			121


/*地域2图层*/
#define TIMEZONE_ADDR2_ORIGIN_X		6
#define TIMEZONE_ADDR2_ORIGIN_Y		90
#define TIMEZONE_ADDR2_SIZE_H		16
#define TIMEZONE_ADDR2_SIZE_W		121


/*差距图层*/
#define TIMEZONE_OFFSET_ORIGIN_X	3
#define TIMEZONE_OFFSET_ORIGIN_Y	157
#define TIMEZONE_OFFSET_SIZE_H		16
#define TIMEZONE_OFFSET_SIZE_W		165

/*地域1时间图层*/
#define TIMEZONE_TIME1_ORIGIN_X		6
#define TIMEZONE_TIME1_ORIGIN_Y		36
#define TIMEZONE_TIME1_SIZE_H		31
#define TIMEZONE_TIME1_SIZE_W		26

/*地域2时间图层*/
#define TIMEZONE_TIME2_ORIGIN_X		6
#define TIMEZONE_TIME2_ORIGIN_Y		115
#define TIMEZONE_TIME2_SIZE_H		31
#define TIMEZONE_TIME2_SIZE_W		26
...
int32_t display_target_layerText (P_Window p_window,GRect temp_text_frame,char *text_str,enum GAlign how_to_align,enum GColor black_or_white, enum GColor fcolor)
{
	int32_t g_layer_id_temporary;
	GBitmap temp_GBitmap = {0};
	
	LayerText layer_text_struct_lg = {text_str, temp_text_frame, how_to_align, fcolor, U_GBK_SIMSUN_16};
...
P_Window double_timezone_create_window_handle(P_Window p_window,int addr_origin_y,int time_origin_y,int flag_sec)
{
	...
	enum GColor time_color = GColorWhite;
	...
	display_target_layerText(p_window,frame_addr1,zone_addr_temp,GAlignLeft,addr_color,GColorBlack);
	/*创建时区1日期文本图层*/
	frame_addr1.origin.x = 129;
	frame_addr1.size.w = 45;
	display_target_layerText(p_window,frame_addr1,zone_mon_day_temp,GAlignLeft,addr_color,GColorBlack);
	
	
	if(flag_sec == 1)
	{
	/*创建时区2地址文本图层*/
	GRect frame_addr2 = {{TIMEZONE_ADDR2_ORIGIN_X, addr_origin_y}, {TIMEZONE_ADDR2_SIZE_H, TIMEZONE_ADDR2_SIZE_W}};
	display_target_layerText(p_window,frame_addr2,zone_addr_temp,GAlignLeft,addr_color,GColorWhite);
	/*创建时区2日期文本图层*/
	frame_addr2.origin.x = 129;
	frame_addr2.size.w = 45;
	display_target_layerText(p_window,frame_addr2,zone_mon_day_temp,GAlignLeft,addr_color,GColorWhite);
	}
...
P_Window double_timezone_create_window()
{
	...
	display_target_layer(p_window,&frame_bg,GAlignCenter,GColorBlack,bmp_id,10);
	...
	if(g_double_timezone.show_second_flag == 1)
	{
		...
		if(g_double_timezone.zone1_offset != g_double_timezone.zone2_offset)
		{
			...
			display_target_layerText(p_window,frame_offset,str_tmp,GAlignLeft,GColorBlack,GColorWhite);
		}		
	}
