 ...
 *  版本: 1.3.0	 修改用户配置内容含义，设置价格落在上一次价格与最新一次价格之间，则到价提醒。
				...
				程序在前台时，15秒请求一次数据；
				程序在后台时，1分钟请求一次数据；
				3分钟之内没有获取到数据，价格颜色黑底黄字
 *  版本: 1.2.0，新增价格提醒，修改用户配置格式规范:
				外汇配置格式为：外汇对 上限提醒阈值 下限提醒阈值（不同的外汇项用换行的方式隔开）
				价格数据15秒更新一次;
 *  版本：1.1.0，新增价格显示背景提醒，背景红色表示相比前一次价格跌，背景蓝色表示涨，背景黑色表示不变。
 *	版本：1.0.0，初始版本。
				价格数据5秒更新一次;
				价格颜色为白色表示实时价格，价格颜色为黄色表示数据已经1分钟以上没有更新;
 */
...
#define MENU_FRAME_POS_Y                     36
#define MENU_FRAME_SIZE_W                    176
#define MENU_FRAME_SIZE_H                    140

/* 列表项框架 */
#define MENU_ITEM_POS_X                      0
#define MENU_ITEM_POS_Y                      36
#define MENU_ITEM_SIZE_W                     176
#define MENU_ITEM_SIZE_H                     28

/* 时间项 */
#define WEEK_TEXT_POS_X                      0
#define WEEK_TEXT_POS_Y                      9
#define WEEK_TEXT_SIZE_W                     50
#define WEEK_TEXT_SIZE_H                     20

#define DATE_TEXT_POS_X                      50
#define DATE_TEXT_POS_Y                      6
#define DATE_TEXT_SIZE_W                     66
#define DATE_TEXT_SIZE_H                     24

#define TIME_TEXT_POS_X                      116
#define TIME_TEXT_POS_Y                      6
#define TIME_TEXT_SIZE_W                     60
#define TIME_TEXT_SIZE_H                     24

/* 外汇货币对 */
#define EXCHANGE_SYMBOL_POS_X           2
#define EXCHANGE_SYMBOL_POS_Y           8
#define EXCHANGE_SYMBOL_SIZE_W          86
#define EXCHANGE_SYMBOL_SIZE_H          20

/* 外汇卖出价 */
#define EXCHANGE_BID_POS_X              88
#define EXCHANGE_BID_POS_Y              8
#define EXCHANGE_BID_SIZE_W             86
#define EXCHANGE_BID_SIZE_H             20

/*
//外汇买入价
#define EXCHANGE_ASK_POS_X              72
#define EXCHANGE_ASK_POS_Y              22
#define EXCHANGE_ASK_SIZE_W             64
#define EXCHANGE_ASK_SIZE_H             16

//外汇买卖差价
#define EXCHANGE_SPREAD_POS_X           138
#define EXCHANGE_SPREAD_POS_Y           4
#define EXCHANGE_SPREAD_SIZE_W          38
#define EXCHANGE_SPREAD_SIZE_H          16
*/
...
//检测数据是否更新（通过获取数据的时间来判断），如果没有更新，改变价格的显示前景颜色
//当前时间与最近一次数据更新的时间比较，如果大于60秒，改变价格的显示前景颜色
static void is_exchange_updating()
{
	...
	for(i=0;i<MAX_ITEM_NUM;i++)
	{
		...
		{
			g_exchange_item[i].bcolor = GColorBlack;
			g_exchange_item[i].fcolor = GColorYellow;
...
static void watch_time_change_callback(enum SysEventType type, void *context)
{
    ...
    if (type == SysEventTypeTimeChange)
    {
        ...
        add_text_layer(p_window, &g_layer_id_week, text_buf, &frame, GAlignLeft, U_ASCII_ARIALBD_20, GColorGreen, GColorBlack);
        ...
        add_text_layer(p_window, &g_layer_id_date, text_buf, &frame, GAlignCenter, U_ASCII_ARIALBD_24, GColorGreen, GColorBlack);
        ...
        add_text_layer(p_window, &g_layer_id_time, text_buf, &frame, GAlignRight, U_ASCII_ARIALBD_24, GColorGreen, GColorBlack);
...
void refresh_exchange_data(P_Window p_window, int update)
{
	...
	for(i=0; i<MAX_ITEM_NUM; i++)
    {
		...
		*/
		...
		add_text_layer(p_window, &g_layer_id_exchange_symbol[i], g_exchange_item[i].symbol, &text1_frame, GAlignLeft, U_ASCII_ARIAL_20, GColorBlack, GColorWhite);
		add_text_layer(p_window, &g_layer_id_exchange_bid[i], g_exchange_item[i].bid, &text2_frame, GAlignRight, U_ASCII_ARIAL_20, g_exchange_item[i].bcolor, g_exchange_item[i].fcolor);
		//add_text_layer(p_window, &g_layer_id_exchange_ask[i], g_exchange_item[i].ask, &text3_frame, GAlignLeft, U_ASCII_ARIAL_16, GColorBlack, GColorWhite);
		//add_text_layer(p_window, &g_layer_id_exchange_spread[i], g_exchange_item[i].spread, &text4_frame, GAlignLeft, U_ASCII_ARIAL_16, GColorBlack, GColorWhite);
    }
