...
//界面刷新时间
#define RUN_HELPER_REFRESH_TIME     1000
...
typedef struct tag_Run
{
	...
	int32_t  curr_speed;	//实时速度，cm/s
	int32_t  max_speed;	//最高速度，cm/s
...
/*结束界面状态栏时间图层ID*/
static int8_t g_run_finish_status_bar_layer_id = -1;
...
static uint8_t g_remember_number = 0;
...
P_Window run_helper_create_finish_window()
{
	...
	g_run_finish_status_bar_layer_id = display_target_layerText(p_window,&finish_temp_frame,GAlignRight,GColorBlack,buf,U_ASCII_ARIAL_12);
...
void run_helper_timer_callback(date_time_t tick_time, uint32_t millis, void *context)
{
	...
	if (g_run.flag == 1)
	{
		g_run.sys_micro = millis - RUN_HELPER_REFRESH_TIME;
		g_run.flag = 0;
	}
	...
	int32_t cur_calorie = data.calorie - g_run.pre_calorie;
	cur_calorie = cur_calorie < 0 ? 0 : cur_calorie;
	g_run.pre_calorie = data.calorie;
	
	//获取实时速度cm/s
	
	if(realt_speed.flag != 0)
	{
	...
	//更新总距离，总卡路里
		...
		g_run.total_distance += g_run.curr_speed*RUN_HELPER_REFRESH_TIME/1000;
...
else if(g_remember_number % 4 == 0)
{
	...
	if (NULL != p_speed_layer)
	{
		...
		uint32_t hour_speed = g_run.curr_speed == 0 ? 1 : g_run.curr_speed;	
	...
	if (type == SysEventTypeTimeChange)
	{
		...
		if (NULL != p_window)
		{
			if(g_run.state == RUN_HELPER_STATE_FINISH)
			{
				...
				p_hm_layer = app_window_get_layer_by_id(p_window, g_run_finish_status_bar_layer_id);
			}
			else
			{
				p_hm_layer = app_window_get_layer_by_id(p_window, g_run_status_bar_layer_id);
			}
