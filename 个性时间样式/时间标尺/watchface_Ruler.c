...
static uint8_t ORIGIN_X[LAYER_NUM+BACKGROUND_NUM] = {0, 102, 115, 0};
static uint8_t ORIGIN_W[LAYER_NUM+BACKGROUND_NUM] = {128, 13, 13, 128};
static uint8_t ORIGIN_H[LAYER_NUM+BACKGROUND_NUM] = {128, 22, 22, 2};
...
static void app_watch_data()
{
	...
	watch_data[2] = 106-datetime.min/5*7;
...
{
	GRect frame = {{ORIGIN_X[pos], watch_data[2]-ORIGIN_H[pos]}, {ORIGIN_H[pos], ORIGIN_W[pos]}};
	GBitmap bitmap;
	...
	if (pos == LAYER_NUM)
	{
		res_get_user_bitmap(PIC_RULE, &bitmap);
		frame.origin.y = watch_data[2];
	}else
	{
