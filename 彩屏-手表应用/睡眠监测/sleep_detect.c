...
int8_t create_layer_text(P_Window p_window,char* str,GRect frame,enum GAlign align,int8_t font,enum GColor color,enum GColor fcolor)
{
	LayerText lt = {str,frame,align,fcolor,font,0};
...
void create_progress_bar(P_Window p_window,uint8_t date_flag,uint8_t percent,ProgressType type )
{
	...
	//绘制进度条
	GPoint prog_bar_points[4] = { {origin_x + date_flag *22, 48}, 
								  {origin_x+width+ date_flag *22, 48}, 
								  {origin_x+width+ date_flag *22, 131}, 
								  {origin_x+ date_flag *22, 131} };
	...
	if((percent>0)&&(100 >= percent ))
	{
		...
		if((type == TYPE_OLD_IN))
		{
			prog_bar_points[0].y += 3;
			prog_bar_points[1].y += 3;
		}
		
	}
	else
	{
		prog_bar_points[0].y += 81;
		prog_bar_points[1].y += 81;
	}
...
void update_display(void)
{
	...
	app_layer_set_bg_color(layer_bmp_bg, GColorBlack);
	...
	if(BGM_flag == 0)
	{
		...
		//*p_frame_bmp = 0x0c1a1f1c;  //GRect frame_bmp = {{28,31},{26,12}};
		*p_frame_bmp = 0x11242a26;  //GRect frame_bmp = {{38,42},{36,17}};
		res_get_user_bitmap(array_num[getSleepHours((datetime.wday))/10],&bmp_temp);
		create_layer_bmp(p_new_window,&bmp_temp,frame_bmp,GAlignCenter,GColorBlack);

		frame_bmp.origin.x += 17;
		res_get_user_bitmap(array_num[getSleepHours((datetime.wday))%10],&bmp_temp);
		create_layer_bmp(p_new_window,&bmp_temp,frame_bmp,GAlignCenter,GColorBlack);
		...
		{//如果有睡眠数据
			//显示睡眠时间
			//GRect frame_bmp = {{0,78},{20,176}};
			*p_frame_bmp = 0xB0144E00; 
			...
			create_layer_text(p_new_window,str_buf,frame_bmp,GAlignCenter,U_ASCII_ARIAL_20,GColorWhite,GColorBlack);

			//显示睡眠质量
			//GRect frame_bmp = {{98,119},{34,19}};
			*p_frame_bmp = 0x13227762;
		...
		{
			//设置挡板
			//GRect frame_bmp = {{52,107},{55,37}};
			*p_frame_bmp = 0x25376b34;
	...
	else if(BGM_flag == 1)
	{
		...
		//GRect frame_bmp = {{0,157},{16,176}};
		*p_frame_bmp = 0xb0109d00;
		str_buf[0] = '\0';
		sprintf(str_buf,"7日平均时长%d时%d分",getWeekSleepTime()/60,getWeekSleepTime()%60);

		create_layer_text(p_new_window,str_buf,frame_bmp,GAlignCenter,U_GBK_SIMSUN_16,GColorWhite,GColorBlack);
