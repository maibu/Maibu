...
#define DOUBLE_WEATHER_ZONE_BG_SIZE_H			176
#define DOUBLE_WEATHER_ZONE_BG_SIZE_W			176

/*日期图片图层起始位置*/
#define DOUBLE_WEATHER_ZONE_MONTH_DAY_ORIGIN_X	5
#define DOUBLE_WEATHER_ZONE_MONTH_DAY_ORIGIN_Y	7
#define DOUBLE_WEATHER_ZONE_MONTH_DAY_SIZE_H	16	
#define DOUBLE_WEATHER_ZONE_MONTH_DAY_SIZE_W	10

/*时间图片图层起始位置*/
#define DOUBLE_WEATHER_ZONE_TIME_ORIGIN_X		128
#define DOUBLE_WEATHER_ZONE_TIME_ORIGIN_Y		7
#define DOUBLE_WEATHER_ZONE_TIME_SIZE_H			16	
#define DOUBLE_WEATHER_ZONE_TIME_SIZE_W			10

/*天气图片上图层位置*/
#define DOUBLE_WEATHER_ZONE_WEATHER_TOP_ORIGIN_X	7
#define DOUBLE_WEATHER_ZONE_WEATHER_TOP_ORIGIN_Y	30
#define DOUBLE_WEATHER_ZONE_WEATHER_TOP_SIZE_H		52
#define DOUBLE_WEATHER_ZONE_WEATHER_TOP_SIZE_W		52	

/*天气图片下图层位置*/
#define DOUBLE_WEATHER_ZONE_WEATHER_BOTTOM_ORIGIN_X	7	
#define DOUBLE_WEATHER_ZONE_WEATHER_BOTTOM_ORIGIN_Y	110
#define DOUBLE_WEATHER_ZONE_WEATHER_BOTTOM_SIZE_H	52
#define DOUBLE_WEATHER_ZONE_WEATHER_BOTTOM_SIZE_W	52	

/*城市文本上图层位置*/
#define DOUBLE_WEATHER_ZONE_CITY_TOP_ORIGIN_X		67
#define DOUBLE_WEATHER_ZONE_CITY_TOP_ORIGIN_Y		30
#define DOUBLE_WEATHER_ZONE_CITY_TOP_SIZE_H			20
#define DOUBLE_WEATHER_ZONE_CITY_TOP_SIZE_W			105	

/*城市文本下图层位置*/
#define DOUBLE_WEATHER_ZONE_CITY_BOTTOM_ORIGIN_X	67
#define DOUBLE_WEATHER_ZONE_CITY_BOTTOM_ORIGIN_Y	110
#define DOUBLE_WEATHER_ZONE_CITY_BOTTOM_SIZE_H		20
#define DOUBLE_WEATHER_ZONE_CITY_BOTTOM_SIZE_W		105	


/*温度图片上图层起始位置*/
#define DOUBLE_WEATHER_ZONE_TEMPERATURE_TOP_ORIGIN_X		69
#define DOUBLE_WEATHER_ZONE_TEMPERATURE_TOP_ORIGIN_Y		59
#define DOUBLE_WEATHER_ZONE_TEMPERATURE_TOP_SIZE_H			21
#define DOUBLE_WEATHER_ZONE_TEMPERATURE_TOP_SIZE_W			65

/*温度图片下图层起始位置*/
#define DOUBLE_WEATHER_ZONE_TEMPERATURE_BOTTOM_ORIGIN_X		69
#define DOUBLE_WEATHER_ZONE_TEMPERATURE_BOTTOM_ORIGIN_Y		139
#define DOUBLE_WEATHER_ZONE_TEMPERATURE_BOTTOM_SIZE_H		21
#define DOUBLE_WEATHER_ZONE_TEMPERATURE_BOTTOM_SIZE_W		65	

/*PM2.5图片上图层起始位置*/
#define DOUBLE_WEATHER_ZONE_PM25_TOP_ORIGIN_X				146
#define DOUBLE_WEATHER_ZONE_PM25_TOP_ORIGIN_Y				71
#define DOUBLE_WEATHER_ZONE_PM25_TOP_SIZE_H					9
#define DOUBLE_WEATHER_ZONE_PM25_TOP_SIZE_W					25

/*PM2.5图片下图层起始位置*/
#define DOUBLE_WEATHER_ZONE_PM25_BOTTOM_ORIGIN_X			146
#define DOUBLE_WEATHER_ZONE_PM25_BOTTOM_ORIGIN_Y			150
#define DOUBLE_WEATHER_ZONE_PM25_BOTTOM_SIZE_H				9
#define DOUBLE_WEATHER_ZONE_PM25_BOTTOM_SIZE_W				25
...
#define WEATHER_KEY			"code,temp,pm2_5,info,city"
...
uint32_t get_weather_icon(char * str)
{
	...
	if(strstr(str,WEATHER_SUNNY) != NULL )
	{
		weather_icon_key = RES_BITMAP_WATCHFACE_WEATHER_SUNNY;
	}
	else if(strstr(str,WEATHER_OVERCAST) != NULL  )
	{
		weather_icon_key = RES_BITMAP_WATCHFACE_WEATHER_OVERCAST;
	}
	else if(strstr(str,WEATHER_CLOUDY) != NULL  )
	{
		weather_icon_key = RES_BITMAP_WATCHFACE_WEATHER_CLOUDY;
	}
	else if((strstr(str,WEATHER_FOG_1) != NULL )||(strstr(str,WEATHER_FOG_2) != NULL )||(strstr(str,WEATHER_FOG_3) != NULL ))
	{
	...
	else
	{
		...
		else if(strstr(str,WEATHER_RAIN) != NULL)
		{
...
/*创建并显示文本图层*/
void display_target_layerText(P_Window p_window,GRect  *temp_p_frame,enum GAlign how_to_align,enum GColor color,char * str,uint8_t font_type,enum GColor fcolor)
{
...
/*分析curPM数据，并显示*/
void check_display_curPM_number(P_Window p_window,GRect  *temp_p_frame,char weather_data_num,enum GColor color)
{
	...
	{
		display_target_layer(p_window,temp_p_frame,GAlignLeft,color,&get_others_icon_key_array[0],3);
	}
	else
	{
		int8_t i = curPM_number_show_pic_count-1;
		temp_p_frame->size.w = 8;
		temp_p_frame->origin.x = DOUBLE_WEATHER_ZONE_PM25_TOP_ORIGIN_X + 2*(temp_p_frame->size.w);
		for(; i >= 0 ; i--)
		{
			display_target_layer(p_window,temp_p_frame,GAlignLeft,color,&get_LIT_icon_key[0],g_weather_data[weather_data_num].curPm[i]-'0');
...
/*分析temperature数据，并显示*/
void check_display_temperature_number(P_Window p_window,GRect  *temp_p_frame,char weather_data_num,enum GColor color)
{
	...
	{
		display_target_layer(p_window,temp_p_frame,GAlignLeft,color,&get_others_icon_key_array[0],2);
	}
	else
	{
		uint8_t i = 0;
		if(g_weather_data[weather_data_num].temperature[0] == '-')
		{
			...
			display_target_layer(p_window,temp_p_frame,GAlignLeft,color,&get_BIG_icon_key[0],10);
			temp_p_frame->origin.x = temp_p_frame->origin.x + 15;
			i = 1;
		}
		temp_p_frame->size.w = 21;
		for(; i < temperature_show_pic_count ; i++)
		{
			display_target_layer(p_window,temp_p_frame,GAlignLeft,color,&get_BIG_icon_key[0],g_weather_data[weather_data_num].temperature[i] - '0');
			temp_p_frame->origin.x = temp_p_frame->origin.x + 21;
		}
		temp_p_frame->size.w = 8;
		display_target_layer(p_window,temp_p_frame,GAlignLeft,color,&get_BIG_icon_key[0],11);
	}
...
P_Window init_window(void)
{
	...
	display_target_layer(p_window,&temp_frame,GAlignLeft,GColorBlack,get_others_icon_key_array,0);


	struct date_time datetime;
	app_service_get_datetime(&datetime);
#if 1
	temp_frame.origin.x = 3;
	temp_frame.origin.y = 3;
	temp_frame.size.h = 16;
	temp_frame.size.w = 171;

	char buffer_time_str[32]={0};
	
	
	sprintf(buffer_time_str, "%02d-%02d                    %02d:%02d", datetime.mon,datetime.mday,datetime.hour,datetime.min);
	display_target_layerText(p_window,&temp_frame,GAlignCenter,GColorBlack,buffer_time_str,U_ASCII_ARIAL_16,GColorWhite);
#else
	//month
	...
	display_target_layerText(p_window,&temp_frame,GAlignLeft,GColorBlack,g_weather_data[TOP_DATA_NUM].city,U_ASCII_ARIAL_20,GColorWhite);
	...
	check_display_temperature_number(p_window,&temp_frame,TOP_DATA_NUM,GColorBlack);
	...
	check_display_curPM_number(p_window,&temp_frame,TOP_DATA_NUM,GColorWhite);
	...
	display_target_layerText(p_window,&temp_frame,GAlignLeft,GColorWhite,g_weather_data[BOTTOM_DATA_NUM].city,U_ASCII_ARIAL_20,GColorBlack);
	...
	check_display_temperature_number(p_window,&temp_frame,BOTTOM_DATA_NUM,GColorWhite);
	...
	check_display_curPM_number(p_window,&temp_frame,BOTTOM_DATA_NUM,GColorBlack);
...
{	
	...
	maibu_get_json_str(buff, "city", &m_back_city ,sizeof(m_back_city));
	...
	maibu_get_json_int(buff, "temp", &temp_number);

	sprintf(g_weather_data[m_weather_data_num].temperature,"%d",temp_number);	
	
	maibu_get_json_str(buff, "pm2_5",g_weather_data[m_weather_data_num].curPm,sizeof(g_weather_data[m_weather_data_num].curPm));
