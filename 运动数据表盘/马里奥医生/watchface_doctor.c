...
#define TIME_NUM 9 //时间位数

/*图片坐标：第一项为背景图片，后面依次为时间的1~4位*/
static uint8_t ORIGIN_X[10] = {0, 85, 93, 105, 113, 104, 112, 104, 112, 117};
static uint8_t ORIGIN_Y[10] = {0, 16, 16, 16, 16, 79, 79, 95, 95, 37};
static uint8_t ORIGIN_H[10] = {128, 6, 6, 6, 6, 6, 6, 6, 6, 6};
static uint8_t ORIGIN_W[10] = {128, 8, 8, 8, 8, 8, 8, 8, 8, 6};


/*图片名数组：第一项为背景图片，后面依次为数字0~9图片*/
uint16_t bmp_array[12] = {PIC_0_1, PIC_1_1, PIC_2_1, PIC_3_1, PIC_4_1, PIC_5_1, PIC_6_1, PIC_7_1, PIC_8_1, PIC_9_1, PIC_A, PIC_P};


/*小时分钟月日图层ID，通过该图层ID获取图层句柄*/
static int8_t g_layer_time[TIME_NUM] ={ -1, -1, -1, -1, -1, -1, -1, -1, -1};

/*窗口ID, 通过该窗口ID获取窗口句柄*/
static int32_t g_window = -1;

/*由系统时间转换为当前表盘应显示时间*/
static int8_t watch_time[TIME_NUM] = {0,0,0,0,0,0,0,0,0};
...
static void app_watch_time()
{
	...
	watch_time[0] = datetime.hour%12/10;
	watch_time[1] = datetime.hour%12%10;
	watch_time[2] = datetime.min/10;
	watch_time[3] = datetime.min%10;
	watch_time[4] = datetime.mon/10;
	watch_time[5] = datetime.mon%10;
	watch_time[6] = datetime.mday/10;
	watch_time[7] = datetime.mday%10;

	if ((watch_time[0] == 0) && (watch_time[1] == 0))
	{
		watch_time[0] = 1;
		watch_time[1] = 2;
	}
