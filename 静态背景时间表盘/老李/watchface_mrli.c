...
#define TIME_NUM 3 //时间位数

/*图片坐标：第一项为背景图片，后面依次为时间的1~4位*/
static uint8_t ORIGIN_X[4] = {0, 79, 91, 115};
static uint8_t ORIGIN_Y[4] = {0, 38, 38, 38};
static uint8_t ORIGIN_H[4] = {128, 16, 16, 16};
static uint8_t ORIGIN_W[4] = {128, 11, 11, 11};


/*图片名数组：第一项为背景图片，后面依次为数字0~9图片*/
uint16_t bmp_array[12] = {PIC_MONEY, PIC_1_1, PIC_2_1, PIC_3_1, PIC_4_1, PIC_5_1, PIC_6_1, PIC_7_1, PIC_8_1, PIC_9_1, PIC_10_1, PIC_0_1};


/*小时分钟月日图层ID，通过该图层ID获取图层句柄*/
static int8_t g_layer_time[TIME_NUM] ={ -1, -1, -1};

/*窗口ID, 通过该窗口ID获取窗口句柄*/
static int32_t g_window = -1;

/*由系统时间转换为当前表盘应显示时间*/
static int8_t watch_time[TIME_NUM] = {0,0,0};
...
static void app_watch_time()
{
	...
	watch_time[0] = 11;
	watch_time[2] = (datetime.min+5)/10;
	if (watch_time[2] == 6)
	{
		datetime.hour = datetime.hour+1;
		watch_time[2] = 0;
	}
	watch_time[1] = datetime.hour%12%11;
	if (datetime.hour%12 == 0 || datetime.hour%12 == 11)
	{
		watch_time[0] = 10;
		watch_time[1] = (datetime.hour%12+2)%12;
	}
}
