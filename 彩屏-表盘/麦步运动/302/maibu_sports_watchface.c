	...
	for(loop_count = max_num_long-1;loop_count >= 0;loop_count--)
	{
		...
		{	
			if(alignment == GAlignCenter)
			{
				if(flag == 0)
				{
					temp_p_frame->origin.x = (176-(loop_count+1)*(temp_p_frame->size.w))/2;
				}
			}
...
static P_Window init_window(void)
{
	...
	temp_frame.origin.y = 77;
	temp_frame.size.w = 22;
	...
	/*添加热量标签文本图层*/
	temp_frame.origin.x = 8;
	temp_frame.origin.y = 128;
	temp_frame.size.w = 50;
	temp_frame.size.h = 16;
	
	sprintf(buffer, "%s", "热量");
	set_temp_layerText(&temp_LayerText, buffer,temp_frame, GAlignLeft,U_ASCII_ARIAL_16,p_window,temp_P_Layer,GColorWhite,GColorBlack);
	
			
	/*添加热量图层*/
	temp_frame.origin.x = 8;
	temp_frame.origin.y = 149;
